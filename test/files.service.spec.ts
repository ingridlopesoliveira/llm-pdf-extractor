...
describe('FilesService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [FilesService, { provide: LLMService, useValue: { extractInvoice: jest.fn() } }, { provide: InvoicesRepository, useValue: { create: jest.fn(), findAll: jest.fn() } }],
    }).compile()
  ...
  it('computes aggregated values correctly', () => {
    ...
    expect(dto.energyConsume).toBe(250) // 10 + 20
    expect(dto.energyCompensated).toBe(0)
    expect(dto.totalValueWithoutGd).toBe(2400) // 100 + 200 + 30
    expect(dto.economyGd).toBe(40)
  })
  ...
  it('getFilesData returns repository results', async () => {
    ...
    await expect(service.getFilesData({ page: 1, pageSize: 10 })).resolves.toBe(arr)
  })
})
